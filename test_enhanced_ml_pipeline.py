#!/usr/bin/env python3
"""
Enhanced ML Pipeline Test
Tests the enhanced model trainer with 16+ algorithms
"""

import sys
import os
import time
from datetime import datetime, timedelta
import pandas as pd
import numpy as np
import sqlite3

# Add project root to Python path
project_root = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, project_root)

def test_enhanced_ml_pipeline():
    """Test enhanced ML pipeline with 16+ algorithms"""
    print("🧪 Enhanced ML Pipeline Test - 16+ Algorithms")
    print("=" * 60)
    print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    try:
        # Import enhanced components
        from main.pipeline.enhanced_model_trainer import EnhancedModelTrainer
        from main.pipeline.data_processor import DataProcessor
        
        # Test database connection and data retrieval
        print("🔍 Testing database connection and data retrieval...")
        db_path = "stock_data.db"
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        # Check available data
        cursor.execute('''
            SELECT interval_type, COUNT(*) as record_count, 
                   MIN(datetime) as earliest_date, 
                   MAX(datetime) as latest_date
            FROM angel_one_stock_data 
            WHERE ticker = 'RELIANCE'
            GROUP BY interval_type
            ORDER BY record_count DESC
        ''')
        
        available_data = cursor.fetchall()
        print(f"📊 Available data in database:")
        for interval, count, earliest, latest in available_data:
            print(f"   {interval}: {count} records ({earliest} to {latest})")
        
        if not available_data:
            print("❌ No data found in database!")
            return False
        
        # Use the interval with most data for ML training
        best_interval = available_data[0][0]
        print(f"✅ Using {best_interval} data for ML training ({available_data[0][1]} records)")
        
        # Retrieve data from database
        print(f"\n📥 Retrieving {best_interval} data from database...")
        cursor.execute('''
            SELECT datetime, open_price, high_price, low_price, close_price, volume
            FROM angel_one_stock_data 
            WHERE ticker = 'RELIANCE' AND interval_type = ?
            ORDER BY datetime
        ''', (best_interval,))
        
        db_data = cursor.fetchall()
        print(f"✅ Retrieved {len(db_data)} records from database")
        
        # Convert to DataFrame
        df = pd.DataFrame(db_data, columns=['datetime', 'open', 'high', 'low', 'close', 'volume'])
        df['datetime'] = pd.to_datetime(df['datetime'])
        df.set_index('datetime', inplace=True)
        
        # Convert to numeric
        for col in ['open', 'high', 'low', 'close', 'volume']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        
        print(f"📊 Data shape: {df.shape}")
        print(f"📅 Date range: {df.index.min()} to {df.index.max()}")
        
        # Show sample data
        print(f"📈 Sample data (first 3 records):")
        for i, (idx, row) in enumerate(df.head(3).iterrows()):
            print(f"   {idx}: Open={row['open']:.2f}, High={row['high']:.2f}, Low={row['low']:.2f}, Close={row['close']:.2f}, Volume={row['volume']:.0f}")
        
        conn.close()
        
        # Initialize Enhanced ML components
        print(f"\n🤖 Initializing Enhanced ML components...")
        
        # Data Processor
        print("   📊 Initializing Data Processor...")
        data_processor = DataProcessor(ticker="RELIANCE")
        
        # Process data with technical indicators
        print("   🔄 Processing data with technical indicators...")
        processed_data = data_processor.execute(data=df, period="1mo", interval="ONE_MINUTE")
        
        if not processed_data.get('success', False):
            print(f"❌ Data processing failed: {processed_data.get('error', 'Unknown error')}")
            return False
        
        # Get the processed data from the response
        if 'enhanced_data' in processed_data:
            processed_df = processed_data['enhanced_data']
        elif 'cleaned_data' in processed_data:
            processed_df = processed_data['cleaned_data']
        elif 'raw_data' in processed_data:
            processed_df = processed_data['raw_data']
        else:
            print(f"❌ No processed data found in response")
            print(f"Available keys: {list(processed_data.keys())}")
            return False
        
        print(f"✅ Data processed successfully: {processed_df.shape}")
        
        # Show processed data features
        print(f"📊 Processed data columns: {list(processed_df.columns)}")
        print(f"📈 Sample processed data (last 3 records):")
        for i, (idx, row) in enumerate(processed_df.tail(3).iterrows()):
            close_val = row.get('Close', row.get('close', 'N/A'))
            volume_val = row.get('Volume', row.get('volume', 'N/A'))
            print(f"   {idx}: Close={close_val}, Volume={volume_val}")
            if 'SMA_20' in row:
                print(f"      SMA_20={row['SMA_20']:.2f}, RSI={row.get('RSI', 'N/A')}")
        
        # Enhanced Model Trainer
        print(f"\n🧠 Initializing Enhanced Model Trainer with 16+ algorithms...")
        enhanced_trainer = EnhancedModelTrainer(ticker="RELIANCE")
        
        # Train all models
        print("   🔄 Training 16+ advanced models...")
        start_time = time.time()
        training_results = enhanced_trainer.execute(data=processed_df)
        training_time = time.time() - start_time
        
        if not training_results.get('success', False):
            print(f"❌ Enhanced model training failed: {training_results.get('error', 'Unknown error')}")
            return False
        
        print(f"✅ Enhanced model training successful!")
        print(f"⏱️ Training time: {training_time:.2f} seconds")
        print(f"📊 Models trained: {training_results.get('models_trained', 0)}")
        print(f"🏆 Best model: {training_results.get('best_model_name', 'Unknown')}")
        print(f"🎯 Best model score: {training_results.get('best_model_score', 0):.4f}")
        
        # Show model performance
        if 'summary' in training_results and 'model_performance' in training_results['summary']:
            print(f"\n🎯 Model Performance Results:")
            print("-" * 40)
            model_perf = training_results['summary']['model_performance']
            for model_name, metrics in model_perf.items():
                r2 = metrics.get('r2', 0)
                mse = metrics.get('mse', 0)
                mae = metrics.get('mae', 0)
                overfitting = metrics.get('overfitting', 0)
                print(f"   {model_name}: R²={r2:.4f}, MSE={mse:.4f}, MAE={mae:.4f}, Overfitting={overfitting:.4f}")
        
        # Show training time for each model
        if 'summary' in training_results and 'training_time' in training_results['summary']:
            print(f"\n⏱️ Training Time per Model:")
            print("-" * 30)
            training_times = training_results['summary']['training_time']
            for model_name, time_taken in training_times.items():
                print(f"   {model_name}: {time_taken:.2f}s")
        
        # Show feature importance for best models
        if 'summary' in training_results and 'feature_importance' in training_results['summary']:
            print(f"\n📊 Feature Importance (Top 5 features for best models):")
            print("-" * 50)
            feature_importance = training_results['summary']['feature_importance']
            for model_name, importance in feature_importance.items():
                if importance:
                    # Sort features by importance
                    sorted_features = sorted(importance.items(), key=lambda x: x[1], reverse=True)
                    print(f"   {model_name}:")
                    for feature, imp in sorted_features[:5]:
                        print(f"      {feature}: {imp:.4f}")
        
        return {
            'success': True,
            'data_retrieved': len(db_data),
            'data_processed': processed_df.shape[0],
            'models_trained': training_results.get('models_trained', 0),
            'best_model': training_results.get('best_model_name', 'Unknown'),
            'best_score': training_results.get('best_model_score', 0),
            'training_time': training_time,
            'training_results': training_results
        }
        
    except Exception as e:
        print(f"❌ Enhanced ML pipeline test failed: {e}")
        import traceback
        traceback.print_exc()
        return None

def generate_enhanced_ml_report(results):
    """Generate enhanced ML report"""
    print("\n" + "=" * 80)
    print("📊 ENHANCED ML PIPELINE TEST REPORT - 16+ ALGORITHMS")
    print("=" * 80)
    
    if not results or not results.get('success', False):
        print("❌ No successful results to report")
        return
    
    print(f"\n📈 Enhanced ML Pipeline Results:")
    print("-" * 40)
    print(f"   Data Retrieved: {results.get('data_retrieved', 0)} records")
    print(f"   Data Processed: {results.get('data_processed', 0)} records")
    print(f"   Models Trained: {results.get('models_trained', 0)} models")
    print(f"   Best Model: {results.get('best_model', 'Unknown')}")
    print(f"   Best Score: {results.get('best_score', 0):.4f}")
    print(f"   Training Time: {results.get('training_time', 0):.2f} seconds")
    
    # Training results
    training_results = results.get('training_results', {})
    if training_results:
        print(f"\n🧠 Enhanced Model Training Results:")
        print("-" * 35)
        
        if 'summary' in training_results and 'model_performance' in training_results['summary']:
            model_perf = training_results['summary']['model_performance']
            for model_name, metrics in model_perf.items():
                r2 = metrics.get('r2', 0)
                mse = metrics.get('mse', 0)
                mae = metrics.get('mae', 0)
                overfitting = metrics.get('overfitting', 0)
                print(f"   {model_name}: R²={r2:.4f}, MSE={mse:.4f}, MAE={mae:.4f}, Overfitting={overfitting:.4f}")
        
        if 'summary' in training_results and 'training_time' in training_results['summary']:
            print(f"\n⏱️ Training Time per Model:")
            print("-" * 30)
            training_times = training_results['summary']['training_time']
            for model_name, time_taken in training_times.items():
                print(f"   {model_name}: {time_taken:.2f}s")
    
    # Save detailed report
    report_file = "docs/ENHANCED_ML_PIPELINE_REPORT.md"
    os.makedirs("docs", exist_ok=True)
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# Enhanced ML Pipeline Test Report - 16+ Algorithms\n\n")
        f.write(f"**Test Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write(f"**Data Retrieved:** {results.get('data_retrieved', 0)} records\n")
        f.write(f"**Data Processed:** {results.get('data_processed', 0)} records\n")
        f.write(f"**Models Trained:** {results.get('models_trained', 0)} models\n")
        f.write(f"**Best Model:** {results.get('best_model', 'Unknown')}\n")
        f.write(f"**Best Score:** {results.get('best_score', 0):.4f}\n")
        f.write(f"**Training Time:** {results.get('training_time', 0):.2f} seconds\n\n")
        
        f.write("## Model Performance Results\n\n")
        if training_results and 'summary' in training_results and 'model_performance' in training_results['summary']:
            model_perf = training_results['summary']['model_performance']
            for model_name, metrics in model_perf.items():
                f.write(f"### {model_name}\n\n")
                f.write(f"- **R² Score**: {metrics.get('r2', 0):.4f}\n")
                f.write(f"- **MSE**: {metrics.get('mse', 0):.4f}\n")
                f.write(f"- **MAE**: {metrics.get('mae', 0):.4f}\n")
                f.write(f"- **Overfitting**: {metrics.get('overfitting', 0):.4f}\n\n")
        
        f.write("## Training Time per Model\n\n")
        if training_results and 'summary' in training_results and 'training_time' in training_results['summary']:
            training_times = training_results['summary']['training_time']
            for model_name, time_taken in training_times.items():
                f.write(f"- **{model_name}**: {time_taken:.2f} seconds\n")
    
    print(f"\n📄 Detailed report saved to: {report_file}")

def main():
    """Main test function"""
    try:
        print("🧪 Enhanced ML Pipeline Test - 16+ Algorithms")
        print("=" * 60)
        
        # Run the enhanced ML pipeline test
        results = test_enhanced_ml_pipeline()
        
        if results:
            # Generate comprehensive report
            generate_enhanced_ml_report(results)
            
            # Calculate overall success
            overall_success = results.get('success', False)
            
            print(f"\n🎯 Overall Result: {'✅ ENHANCED ML PIPELINE SUCCESS' if overall_success else '❌ ENHANCED ML PIPELINE FAILED'}")
            
            if overall_success:
                print("\n🎉 Enhanced ML pipeline test successful!")
                print("✅ Data retrieved from database")
                print("✅ Data processed with technical indicators")
                print(f"✅ {results.get('models_trained', 0)} advanced models trained")
                print(f"✅ Best model: {results.get('best_model', 'Unknown')} (R² = {results.get('best_score', 0):.4f})")
                print("✅ Comprehensive report generated")
                print("\n🚀 Enhanced ML pipeline with 16+ algorithms is fully functional!")
            else:
                print("\n❌ Enhanced ML pipeline test failed!")
                print("❌ Check database data, ML components, and model training")
                print("❌ Review detailed report for specific issues")
            
            return overall_success
        else:
            print("❌ Test failed")
            return False
        
    except Exception as e:
        print(f"❌ Testing failed: {e}")
        import traceback
        traceback.print_exc()
        return False

if __name__ == "__main__":
    success = main()
    sys.exit(0 if success else 1)
