#!/usr/bin/env python3
"""
Incremental Learning Module for AI Stock Predictor

This module provides incremental learning capabilities including:
...
class IncrementalLearningManager:
    """Manages incremental learning for stock prediction models"""
...
class IncrementalTrainingPipeline:
    """Pipeline for incremental model training"""
    
    def __init__(self, learning_manager: IncrementalLearningManager):
    ...
    def update_model_incrementally(self, ticker: str, mode: str, 
        ...
        if should_update:
            ...
            # Backup current model
            backup_path = self.learning_manager.backup_current_model(ticker, mode)
...
class ContinuousLearningScheduler:
    """Scheduler for continuous learning updates"""
    
    def __init__(self, learning_manager: IncrementalLearningManager):
        self.learning_manager = learning_manager
        self.update_schedule = {}  # ticker -> next_update_time
        
    def schedule_update(self, ticker: str, mode: str, 
                       update_frequency_days: int = 7):
        """Schedule regular updates for a model"""
        next_update = datetime.now() + timedelta(days=update_frequency_days)
        self.update_schedule[f"{ticker}_{mode}"] = next_update
        
        logger.info(f"Scheduled update for {ticker} ({mode}) on {next_update}")
    
    def get_due_updates(self) -> List[Tuple[str, str]]:
        """Get list of models due for updates"""
        due_updates = []
        current_time = datetime.now()
        
        for key, next_update in self.update_schedule.items():
            if current_time >= next_update:
                ticker, mode = key.split('_', 1)
                due_updates.append((ticker, mode))
        
        return due_updates
    
    def mark_update_completed(self, ticker: str, mode: str):
        """Mark an update as completed and reschedule"""
        key = f"{ticker}_{mode}"
        if key in self.update_schedule:
            del self.update_schedule[key]
            logger.info(f"Marked update completed for {ticker} ({mode})")


# Utility functions
def create_incremental_learning_manager(base_path: str = "models/incremental") -> IncrementalLearningManager:
    """Create and return an incremental learning manager"""
    return IncrementalLearningManager(base_path)

def create_incremental_training_pipeline(learning_manager: IncrementalLearningManager) -> IncrementalTrainingPipeline:
    """Create and return an incremental training pipeline"""
    return IncrementalTrainingPipeline(learning_manager)

def create_continuous_learning_scheduler(learning_manager: IncrementalLearningManager) -> ContinuousLearningScheduler:
    """Create and return a continuous learning scheduler"""
    return ContinuousLearningScheduler(learning_manager)
