#!/usr/bin/env python3
"""
Test script for parallel model training functionality
"""
...
def test_error_handling():
    """Test error handling in parallel training"""
    print("\n🛡️ Testing Error Handling in Parallel Training")
    print("=" * 50)
    
    # Create problematic data that might cause errors
    data = create_sample_data(n_samples=50, n_features=5)  # Small dataset
    
    config = {
        'random_state': 42,
        'cv_folds': 5,  # High CV folds for small dataset
        'test_size': 0.3,  # Large test size
        'enable_parallel_training': True,
        'max_workers': 2,
        'use_process_pool': False,  # Use ThreadPool for easier debugging
        'enable_cross_validation': True,
        'enable_ensemble': False
    }
    
    trainer = ModelTrainer("TEST_ERROR", config)
