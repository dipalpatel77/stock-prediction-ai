...
# Suppress all warnings and ML library verbose output
...
os.environ['XGBOOST_LOG_LEVEL'] = 'FATAL'
import talib
...
class InteractiveDataSelector:
    ...
    def select_data_period(self, max_days):
        ...
        while True:
            try:
                ...
                if choice in suggestions and suggestions[choice][0] <= max_days:
                    selected_days = suggestions[choice][0]
                    ...
                    if 1 <= custom_days <= max_days:
                        ...
                        print(f"❌ Please enter between 1 and {max_days:,}")
                else:
    ...
    def select_training_data_size(self, total_days):
        ...
        while True:
            try:
                ...
                if choice in suggestions and suggestions[choice][0] <= total_days:
                    selected_days = suggestions[choice][0]
                    print(f"✅ Selected: {suggestions[choice][1]}")
                    return selected_days
                elif choice == '6':
                    custom_days = int(input(f"🎯 Enter custom training days (1-{total_days:,}): "))
                    if 1 <= custom_days <= total_days:
                        print(f"✅ Selected: {custom_days:,} days training")
                        return custom_days
                    else:
                        print(f"❌ Please enter between 1 and {total_days:,}")
                else:
                    print("❌ Invalid choice")
            except ValueError:
                print("❌ Please enter a valid number")
    ...
    def select_prediction_horizon(self):
        ...
        suggestions = {
            '1': (1, '1 day (Tomorrow)'),
            '2': (7, '1 week (Short-term)'),
            '3': (30, '1 month (Medium-term)'),
            '4': (90, '3 months (Long-term)'),
            '5': (365, '1 year (Very long-term)')
        }
        
        print("💡 SUGGESTED HORIZONS:")
        for key, (days, description) in suggestions.items():
            print(f"   {key}. {description}")
        ...
        while True:
            try:
                ...
                if choice in suggestions:
                    ...
                    custom_days = int(input("🔮 Enter custom prediction days (1-365): "))
                    if 1 <= custom_days <= 365:
                        print(f"✅ Selected: {custom_days} days prediction")
                        return custom_days
                    else:
                        print("❌ Please enter between 1 and 365")
                else:
                    print("❌ Invalid choice")
            except ValueError:
                print("❌ Please enter a valid number")
    ...
    def display_summary(self, interval, data_days, training_days, prediction_days):
        ...
        print("=" * 40)
        print(f"📈 Interval: {self.interval_descriptions[interval]}")
...
try:
    from src.core.currency_service import format_price, format_change, get_currency_symbol
...
try:
    from src.utils.enhanced_date_utils import DateUtils
...
class UnifiedAnalysisPipeline:
    ...
    def _get_angel_one_config(self):
        """Get Angel One configuration for Indian stocks."""
        try:
            from config.angel_one_config import get_angel_one_config
            return get_angel_one_config('default')
        except ImportError:
            return {
    ...
    def _is_indian_stock(self, ticker):
        ...
        return ticker.endswith('.NS') or ticker in [
            'TCS', 'RELIANCE', 'HDFC', 'INFY', 'WIPRO', 'HCLTECH', 
            'BHARTIARTL', 'ITC', 'SBIN', 'ICICIBANK', 'KOTAKBANK',
            'AXISBANK', 'MARUTI', 'TITAN', 'NESTLEIND', 'ULTRACEMCO',
            'TECHM', 'POWERGRID', 'NTPC', 'ONGC', 'COALINDIA',
            'SUNPHARMA', 'DRREDDY', 'CIPLA', 'BAJFINANCE', 'BAJAJFINSV',
            'ASIANPAINT', 'TATAMOTORS', 'TATASTEEL', 'JSWSTEEL', 'GALAXYSURF'
        ]
    ...
    def run_unified_analysis(self, period=None, days_ahead=5, use_enhanced=True):
        ...
        try:
            ...
            # Step 6: Prediction Summary (Already generated in Step 2)
            print("\n🔮 Step 6: Prediction Summary")
            print("-" * 50)
            print("✅ Predictions already generated in Step 2 (Model Training)")
            print("📊 Check generated prediction files in data/ directory")
        ...
        try:
            ...
            models = {
                ...
                'LightGBM': lgb.LGBMRegressor(
                    n_estimators=300, learning_rate=0.05, max_depth=10,
                    subsample=0.8, colsample_bytree=0.8, random_state=42,
                    reg_alpha=0.1, reg_lambda=1.0, verbose=-1, force_col_wise=True
                ),
            ...
            for name, model in models.items():
                ...
                try:
                    # Define training function with warning suppression
                    def train_model():
                        import warnings
                        import sys
                        import os
                        ...
                        with warnings.catch_warnings():
                            ...
                            elif name in ['SVR', 'Ridge', 'Lasso', 'ElasticNet', 'Huber', 'KernelRidge', 'MLP', 'GaussianProcess']:
...
def main():
    ...
    # Initialize pipeline first to check Indian stock
    pipeline = UnifiedAnalysisPipeline(ticker)
    
    # Check if it's an Indian stock and offer Angel One configuration
    if pipeline._is_indian_stock(ticker):
        ...
        if use_angel_one != 'n':
            ...
            # Store Angel One config in pipeline
            pipeline.angel_config = {
            ...
            print("📊 Using Yahoo Finance for Indian stock data")
    else:
    ...
    if analysis_choice == "2":
        ...
        forecast_type = "interactive"
        days_ahead = prediction_days
        
        print(f"\n🎯 Starting interactive analysis for {ticker}...")
        print(f"📅 Data period: {period} ({data_days} days)")
        print(f"📈 Interval: {interval}")
        print(f"🎯 Training data: {training_days} days")
        print(f"⏰ Prediction horizon: {days_ahead} days")
        
    else:
        # Standard forecast type selection
        ...
        # Map forecast choice to parameters
        forecast_map = {
            "1": ("1mo", 1, "intraday"),
            "2": ("3mo", 7, "short_term"), 
            "3": ("6mo", 28, "medium_term"),
            "4": ("1y", 365, "long_term"),
            "5": ("2y", 365, "comprehensive"),
            "6": ("1y", 365, "multi_timeframe")
        }
        
        period, days_ahead, forecast_type = forecast_map.get(forecast_choice, ("1y", 365, "multi_timeframe"))
    ...
    if forecast_type == "interactive":
        # Run interactive analysis with custom parameters
        print(f"\n🔧 STEP 1: DATA LOADING WITH CUSTOM PARAMETERS")
        print("-" * 50)
        
        # Load data with custom interval and period
        try:
            # Use enhanced Angel One service for custom data loading
            from src.core.enhanced_angel_one_service import EnhancedAngelOneService
            angel_service = EnhancedAngelOneService()
            
            # Get ticker without suffix for Angel One
            clean_ticker = ticker.replace('.NS', '').replace('.BO', '')
            exchange = "BSE" if 'NS' in ticker else "NSE"
            
            print(f"📊 Loading {data_days} days of {interval} data for {clean_ticker}...")
            ...
            if df is not None and not df.empty:
                ...
                # Store custom data in pipeline
                pipeline.custom_data = df
                pipeline.custom_config = config
                
                # Run analysis with custom data
                success = pipeline.run_unified_analysis(period, days_ahead, use_enhanced)
                
            else:
                print("❌ Failed to load custom data")
                success = False
    ...
    if success:
        ...
        print("   📡 Angel One API → Data Source")
        print("   💾 MySQL Database → Data Storage") 
        print("   🤖 16 ML Models → Predictions")
        print("   📊 Multi-timeframe → Analysis")
        print("   🎯 Trading Signals → Recommendations")
        
        # Comprehensive predictions already generated during analysis
        print("📊 Comprehensive predictions available in data/ directory")
        
        # Display forecast type summary
        if forecast_type == "interactive":
            print("📊 Interactive analysis completed - Check custom interval predictions")
            print(f"📈 Custom configuration: {interval} interval, {data_days} days data, {training_days} days training")
