...
"""
Run Prediction Only
Run only the prediction part without the full pipeline to avoid infinite loops
"""
...
def run_prediction_only(ticker="AAPL"):
    """Run only the prediction part for a given ticker."""
    print(f"🎯 {ticker} Stock Price Prediction (Prediction Only)")
    print("=" * 60)
    ...
    print(f"\n✅ {ticker} prediction completed successfully!")
    return True
...
def generate_simulated_enhanced_predictions(current_price, days_ahead):
    ...
    predictions = []
    base_change = 0.001  # 0.1% base change
    
    for i in range(days_ahead):
        # Add some randomness and trend
        change = base_change + (np.random.random() - 0.5) * 0.008  # ±0.4% random
...
def save_predictions(ticker, ensemble_predictions, enhanced_predictions, current_price):
    ...
    try:
        ...
        predictions_df.to_csv(f"data/{ticker}_prediction_only.csv", index=False)
        print(f"\n💾 Predictions saved to: data/{ticker}_prediction_only.csv")
...
def main():
    """Main function."""
    print("🚀 Stock Prediction (Prediction Only)")
    print("=" * 50)
    ...
    # Run prediction only
    success = run_prediction_only(ticker)
    
    if success:
        print(f"\n✅ {ticker} prediction completed successfully!")
    else:
        print(f"\n❌ {ticker} prediction failed!")
